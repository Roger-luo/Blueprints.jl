...
@option struct ProjectTest <: MustacheBlueprint
    template::String = default_template("test", "runtests.jl")
    project::Bool = false
end

src_file(bp::ProjectTest, ::Context) = bp.template
dst_file(::ProjectTest, ctx::Context) = joinpath(ctx.kwargs.path, "test", "runtests.jl")
